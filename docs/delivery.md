...
Task 41 的两项检查均 timed_out=false，memory.max=999997440，memory.peak 达到上限，oom_kill 从 0 增至 1、再增至 2；同时出现 codec 编译器 signal: killed。容器发生 OOM 已有资源记录支持，计数为容器共享，不能将每次事件严格归因到单个进程。

静态检查已使用 -p=1、GOMAXPROCS=2，现在对子进程增加 GOMEMLIMIT=512MiB，并在执行环境诊断中返回。该值是每个 Go 进程的软预算，不是进程 RSS 硬限制，也不限制 C 编译器或并发任务的总内存。降低编译峰值可能增加 GC 与耗时；若线上仍 OOM，需提升专用执行环境内存或减少并发审查，不能宣称已彻底解决。

令牌超时测试原先创建 httptest 服务但未连接，仍请求外网。现通过仅在测试中注入的 Transport 重定向至本地，检查服务收到请求且错误为 context.DeadlineExceeded。新增 eval/review-regressions 正反例，已参与提示词调整，属于回归集，不作为未见过的 holdout。

审查提示补充核对 mock 的实际接线和断言能否区分无关错误；资源失败后仍继续基于源码证据审查。效果需要 live 复测，离线脚本通过不证明模型已经修复漏查。

本地验证：使用已有同工具链验证镜像，在 --network none、--memory 1000000000、--memory-swap 1000000000、全新 GOCACHE 下，设置上述软预算单独 go build codec v1.3.1 成功，oom_kill=0。memory.peak=1000001536 字节，仍紧贴容器上限，不能据此认为有足够并发余量，也不等于线上全仓检查通过。未做同条件无软预算对照，不能量化该设置的独立收益。
