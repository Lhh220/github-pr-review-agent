...
当前六样本 live 对照各 18/18 完成。原始报告为 report-20260923T011145.367972400Z.json（关闭）和 report-20260923T011318.879329400Z.json（开启），precision/recall 分别为 0.8889 / 0.7778，正常样本误报率均为 0。开启组 17/18 次调用检索；平均 token 15100.9 → 16136.7，耗时 4788.3 → 4286.1 ms。小样本顺序运行不证明稳定提速或质量收益。

001 的构造函数 session/session.go:5 未初始化 map，是明确的修复位置；现增加为精确 accepted_locations，首选位置仍为 session/put.go:4。测试示例 session/session_test.go:3 不接受，其他构造函数行也不接受。模型输出仍需人工核对因果链，标签匹配不等于语义正确。

使用上述标签离线重评分：关闭组 1.0000，开启组 0.8889；主定位指标不变。新报告为 report-20260923T012411.052372100Z.json、report-20260923T012413.056196100Z.json，原始报告保留。分数变化来自标签校准，不是模型改进。

开启组首轮原始 finding 已定位到 Put 的写入，但附的是构造函数和测试示例证据。位置校验触发纠正，旧提示引导重新定位到调用方，最终移到了测试示例。检索结果已包含 Put 的准确源码。现修改位置校验提示：先保留生产缺陷位置，使用已取回的 diff/检索源码补齐对应引用；跨文件辅助证据不要求移动定位，不以测试示例替代生产缺陷。仍不自动改行号、不放宽证据校验。新增测试验证原引用错位触发纠正、同位置真实引用通过，以及构造函数备选位置边界。

提示词修改后的 live A/B 尚未执行；离线测试及旧输出重评分不能证明模型会按新提示纠正。下一轮使用本页 A/B 命令，固定模型和数据集，重点检查 001 的修复前后响应及最终定位。
