...
目标：先有可演示的 MVP，再逐步加工程化和 Agent 质量。test
...
- [x] Day 8 发布恢复：持久化发布标识与 commit、重试复用结果、GitHub 评论对账、发布凭证与 done 同事务、故障注入和 MySQL 本地集成验收；添加 CI 工作流（远端待运行）
...
- [x] CI 增加 Compose 构建、启动、健康与鉴权冒烟；远端待运行
